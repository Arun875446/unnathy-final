...
const HeroCarousel = () => {
  ...
  const animateTransition = (nextIndex: number) => {
    if (isTransitioning.current || nextIndex === currentSlide) return;
    isTransitioning.current = true;
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        // FIX: Update the base layer state first
        setCurrentSlide(nextIndex);

        // FIX: Delay the reset of the top layer by a tiny amount (30ms)
        // This gives the browser enough frames to swap the background image source
        // effectively hiding the "flicker" behind the top layer.
        setTimeout(() => {
    ...
    tl.fromTo(
      topImageRef.current,
      { opacity: 0 },
      { opacity: 1, duration: 0.8, ease: "none" } // "none" or "power1" is better for crossfades
    );
  };
  ...
  return (
    <div className="relative w-full overflow-hidden bg-black aspect-[16/10] md:aspect-auto md:h-[80vh] lg:h-[90vh]">
      {/* LAYER 1: The Base */}
      <div className="absolute inset-0 z-0">
        <img
          key={`base-${currentSlide}`} // Key forces a clean re-render if needed
          src={banners[currentSlide].image}
          className="w-full h-full object-cover"
          alt=""
        />
      ...
      {/* LAYER 2: The Incoming */}
      <div ref={topImageRef} className="absolute inset-0 z-10 opacity-0">
        <img
          key={`top-${nextSlide}`} // Key ensures the GIF starts fresh
          src={banners[nextSlide].image}
          className="w-full h-full object-cover"
          alt=""
        />
